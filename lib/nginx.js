...
const state_store = require('./state_store');
...
module.exports = {
    ...
    write_config(callback) {
        ...
        function remove_config_directories(callback) {
            ...
            function remove_directory(directory, callback) { fs.readdir(directory, (err, files) => {
                    if(err) {
        ...
        function write_vhosts(callback) {
            async.parallel({
                applications: state_store.get_applications,
                firewalls: (callback) => {
                    state_store.get_firewalls((err, firewalls) => {
                        if(err) {
                            return callback();
                        } else {
                ...
                loadbalancers: state_store.get_loadbalancers
            }, (err, response) => {
                if(err) {
                    return callback(err);
                }
                ...
                response.applications = _.keyBy(response.applications, 'id');
